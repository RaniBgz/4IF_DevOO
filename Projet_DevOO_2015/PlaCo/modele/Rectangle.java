...
public class Rectangle extends Forme {
	...
	public boolean contenuDans(Point p1, Point p2) {
		...
		return (coinOppose != null) && (coin.contenuDans(p1, p2)) && (coinOppose.contenuDans(p1, p2));
	}
	...
	private boolean disjoint(Rectangle r){    
        int xOverlap = Math.max(0, 
        		1+Math.min(coin.getX()+largeur,r.getCoin().getX()+r.getLargeur()) -
        		Math.max(coin.getX(),r.getCoin().getX()));
        int yOverlap = Math.max(0, 
        		1+Math.min(coin.getY()+hauteur,r.getCoin().getY()+r.getHauteur()) - 
